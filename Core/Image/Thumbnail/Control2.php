...
class Image_Thumbnail_Control2
{
    ...
    function __construct($minWidth, $minHeight, $image, $imageLink=false)
    {
		...
		if ( isset($_COOKIE[Image_Thumbnail_Control2::CTRLID_NAME]) ) {
			$this->_control = $_COOKIE[Image_Thumbnail_Control2::CTRLID_NAME];
		} else {
			$this->_control = Image_Thumbnail_Control2::CTRLID_PREFIX . time();
			setcookie(Image_Thumbnail_Control2::CTRLID_NAME, $this->_control);
		}
        ...
        // Set minimal width
        if ( $minWidth < Image_Thumbnail_Control2::MIN_WIDTH || $minWidth > $width ) {
            $minWidth = Image_Thumbnail_Control2::MIN_WIDTH;
        }

        // Set minimal height
        if ( $minHeight < Image_Thumbnail_Control2::MIN_HEIGHT || $minHeight > $height ) {
            $minHeight = Image_Thumbnail_Control2::MIN_HEIGHT;
        }
 ...
 * @result ImageTumbnailControl2
 * @access public
 */
function ImageTumbnailControl2(form, image, thumb)
{
