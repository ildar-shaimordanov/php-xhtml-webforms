...
<?
    ...
    function showHtml($image, $options=array())
    {
        ...
        Image_Thumbnail_Control::showJavascript();
...
<?
        ...
        Image_Thumbnail_Control::_showSelect('method', $options, $value_list);

        $value_list = array();
        for ($i = $options['width']; $i <= $maxWidth; $i += 50) {
            $value_list[$i] = $i;
        }
        Image_Thumbnail_Control::_showSelect('width', $options, $value_list);

        $value_list = array();
        for ($i = $options['height']; $i <= $maxHeight; $i += 50) {
            $value_list[$i] = $i;
        }
        Image_Thumbnail_Control::_showSelect('height', $options, $value_list);

        $value_list = array('0.0' => 0, '0.1' => 10, '0.2' => 20, '0.3' => 30, '0.4' => 40, '0.5' => 50, '0.6' => 60, '0.7' => 70, '0.8' => 80, '0.9' => 90, '1.0' => 100);
        Image_Thumbnail_Control::_showSelect('percent', $options, $value_list);

        $value_list = array(-1 => 'Left', 0 => 'Center', +1 => 'Right');
        Image_Thumbnail_Control::_showSelect('halign', $options, $value_list);

        $value_list = array(-1 => 'Top', 0 => 'Center', +1 => 'Bottom');
        Image_Thumbnail_Control::_showSelect('valign', $options, $value_list);
